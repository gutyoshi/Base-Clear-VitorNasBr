...
vRPidd = {}
Tunnel.bindInterface("vitor_admin",vRPidd)
Proxy.addInterface("vitor_admin",vRPidd)
IDDclient = Tunnel.getInterface("vitor_admin")
		...
		PerformHttpRequest(logAdminRenomear, function(err, text, headers) end, 'POST', json.encode({
			embeds = {
				{ 	------------------------------------------------------------
					...
					footer = { 
						text = "NomeDoSeuServidor "..os.date("%d/%m/%Y | %H:%M:%S"),
						icon_url = "COLOCAOLINKDAIMAGEM!"
					},
					color = 15906321 
				}
			}
...
RegisterCommand('fix',function(source,args,rawCommand)
	...
	if vehicle then
		if vRP.hasPermission(user_id,"manager.permissao") or vRP.hasPermission(user_id,"level2.permissao") or vRP.hasPermission(user_id,"level3.permissao") or vRP.hasPermission(user_id,"level4.permissao")then
...
RegisterCommand('nc',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"administrador.permissao") or vRP.hasPermission(user_id,"moderador.permissao") or vRP.hasPermission(user_id,"manager.permissao") then
		...
		TriggerClientEvent("Notify",source,"sucesso"," <b>Você ativou/desativou o noclip</b>!")	
...
RegisterCommand('tptome',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"administrador.permissao") or vRP.hasPermission(user_id,"moderador.permissao") or vRP.hasPermission(user_id,"manager.permissao") then
		if args[1] then
			...
			local x,y,z = vRPclient.getPosition(source)
			if vRP.request(tplayer,"O Passaporte "..source.." quer puxar você até ele, Deseja aceitar ?",30) then
				TriggerClientEvent("Notify",source,"ATENÇÃO","A solicitação foi enviada!")
				if tplayer then
...
RegisterCommand('tpto',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"administrador.permissao") or vRP.hasPermission(user_id,"moderador.permissao") or vRP.hasPermission(user_id,"manager.permissao") then
		if args[1] then
			local tplayer = vRP.getUserSource(parseInt(args[1]))
			if vRP.request(tplayer,"O Passaporte "..source.." quer chegar ate voce! Deseja aceitar ?",30) then
				TriggerClientEvent("Notify",source,"aviso","A solicitação foi enviada!")
				if tplayer then
					PerformHttpRequest(logAdminTps, function(err, text, headers) end, 'POST', json.encode({
						embeds = {
							{ 	------------------------------------------------------------
...
RegisterCommand('tpway',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"dsa.permissao") or vRP.hasPermission(user_id,"dsa.permissao") or vRP.hasPermission(user_id,"dsa.permissao") then
