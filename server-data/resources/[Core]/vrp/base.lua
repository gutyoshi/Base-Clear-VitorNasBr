		...
		if user_id then
			...
			if not vRP.isBanned(user_id) then
				...
				if vRP.isWhitelisted(user_id) then
					...
					deferrals.done("Seu ID é: [ "..user_id.." ]")
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("Você foi banido! [ Mais informações em: discord.gg/ziraflix ] ")
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
