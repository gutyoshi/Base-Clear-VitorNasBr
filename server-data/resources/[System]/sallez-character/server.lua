...
function processSpawnController(source,statusSent,user_id)
	...
	elseif statusSent == 1 or statusSent == 0 then
		userlogin[user_id] = true
		TriggerClientEvent("sallez-character:characterCreate",source)
	end
end

RegisterServerEvent("sallez-character:finishedCharacter")
AddEventHandler("sallez-character:finishedCharacter",function(characterNome,characterSobrenome,characterAge,currentCharacterMode)
	local source = source
...
function doSpawnPlayer(source,user_id,firstspawn)
	TriggerClientEvent("sallez-character:normalSpawn",source,firstspawn)
